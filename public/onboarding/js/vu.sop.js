...
if (typeof vu.sop == 'undefined') {
  vu.sop = function () {};
}

if (typeof vu.face == 'undefined') {
  vu.face = function () {};
}

if (typeof vu.sop.audio == 'undefined') {
  vu.sop.audio = function () {};
}
...
vu.sop.load = async function (basePath) {
  try {
    if (vu.sop.setHEICFileFormatSupport) {
      vu.sop.HEICFileFormatSupportLibLoad = vu.sop.loadJs(
        basePath + '/js/libs/heic2any/heic2any.min.js'
      );
    }

    let htmlLoad = vu.sop.loadHtml(basePath + '/html/onboarding.html');
    let webRTCadapter = vu.sop.loadJs(
      basePath + '/js/libs/webrtc/adapter-latest.js'
    );
    let msgs = vu.sop.loadJs(basePath + '/js/vu.sop.msg.en' + '.js');
    let errors = vu.sop.loadJs(basePath + '/js/vu.error.js');

    let tfJsLoad = vu.sop.loadJs(
      basePath + '/js/libs/tensorflowjs/3.11.0/tf.min.js'
    );
    await tfJsLoad;

    let tfJsWasmLoad = vu.sop.loadJs(
      basePath + '/js/libs/tensorflowjs/3.11.0/tf-backend-wasm.min.js'
    );
    ...
    if (vu.sop.useGestures) {
      ...
      if (vu.sop.preCacheFaceModelAsync) {
        ...
        vu.sop.preCacheFaceModelPromise = vu.sop.loadHtml(
          vu.face.nncPath + '/jeelizFaceTransferNNC.json'
        );
      }
    } else {
      console.log(
        'Challenge orientation model',
        vu.sop.faceOrientationModelWeights
      );
    ...
    if (loadAudioLang) {
      audioLangLoad = vu.sop.loadJs(
        basePath + '/js/vu.sop.audio.' + vu.sop.lang + '.js'
      );
    }
    let audioLoad = vu.sop.loadJs(basePath + '/js/vu.sop.audio.js');
    let cameraLoad = vu.sop.loadJs(basePath + '/js/vu.camera.js');
    let blurDetectionLoad = vu.sop.loadJs(
      basePath + '/js/libs/inspector-bokeh/dist/measure_blur.js'
    );
    let picoLoad = vu.sop.loadJs(basePath + '/js/libs/pico/pico.js');
    let documentLoad = vu.sop.loadJs(
      basePath + '/js/vu.sop.document.objectDetection.js'
    );
    let apiLoad = vu.sop.loadJs(basePath + '/js/vu.sop.api.js');
    let sopUILoad = vu.sop.loadJs(basePath + '/js/vu.sop.ui.js');
    let documentFaceLoad = vu.sop.loadJs(
      basePath + '/js/vu.sop.document.face.js'
    );
    ...
    let h264 = vu.sop.loadJs(
      basePath + '/js/libs/h264-mp4-encoder/h264-mp4-encoder.web.js'
    );
    let htm2canvas = vu.sop.loadJs(
      basePath + '/js/libs/html2canvas/html2canvas.min.js'
    );

    if (vu.sop.useGestures == true) {
      console.log('Loading challenge gestures');
      faceLoad = vu.sop.loadJs(basePath + '/js/vu.face.gestures.js');
      faceUiGesturesLoad = vu.sop.loadJs(
        basePath + '/js/vu.face.ui.gestures.js'
      );
      faceLibLoad = vu.sop.loadJs(
        basePath + '/js/libs/face/jeelizFaceTransfer.js'
      );
      ...
      faceObjectDetection = vu.sop.loadJs(
        basePath + '/js/vu.sop.face.objectDetectionAndRotation.js'
      );
      faceDirectionGesturesDetection = vu.sop.loadJs(
        basePath + '/js/vu.sop.face.model.directionsAndGestures.js'
      );
      faceMixedChallengeUi = vu.sop.loadJs(
        basePath + '/js/vu.face.ui.mixedChallenge.js'
      );
    } else {
      console.log('Loading challenge orientation');
      faceLoad = vu.sop.loadJs(basePath + '/js/vu.face.orientation.js');
      faceLibLoad = vu.sop.loadJs(
        basePath + '/js/libs/face/jeelizFaceFilter.js'
      );
    }
    ...
    vu.sop.document.objectDetection.modelURL =
      basePath + '/js/models/document/model.json';
    vu.sop.document.face.cascadeUrl = basePath + '/js/libs/pico/facefinder.txt';

    document.getElementById('vu.sop.ui.userName').placeholder =
      vu.sop.msg.userInputPlaceholder;
    document.getElementById('vu.sop.ui.userNameSendBtn').innerHTML =
      vu.sop.msg.userSendBtn;
      ...
      if (window.innerHeight > window.innerWidth) {
        ...
        if (vu.camera.isVerticalVideo()) {
          ...
          if (vu.sop.videoResizeStyleFillContainer) {
            console.log(
              'Rules doc - Vertical Screen - Vertical video - fill yes'
            );
            ...
            console.log(
              'Rules doc - Vertical Screen - Vertical video - fill no'
            );
          ...
          if (vu.sop.videoResizeStyleFillContainer) {
            console.log(
              'Rules doc - Vertical Screen - Horizontal video - fill yes'
            );
            ...
            console.log(
              'Rules doc - Vertical Screen - Horizontal video - fill no'
            );
            vid.style.maxWidth = '100%';
            vid.style.maxHeight = 'inherit';
            vid.style.width = 'auto';
        ...
        if (vu.sop.videoResizeStyleFillContainer) {
          console.log(
            'Rules doc - Horizontal Screen - Horizontal video - fill yes'
          );
          vid.style.maxWidth = 'fit-content';
          vid.style.maxHeight = 'inherit';
          vid.style.width = 'auto';
          ...
          console.log(
            'Rules doc - Horizontal Screen - Horizontal video - fill no'
          );
      ...
      if (window.innerHeight > window.innerWidth) {
        ...
        if (vu.camera.isVerticalVideo()) {
          if (vu.sop.videoResizeStyleFillContainer) {
            ...
            console.log(
              'Rules face - Vertical Screen - Vertical video - fill yes'
            );
            // Si el video esta en vertical
            vid.style.maxWidth = '100%';
            ...
            console.log(
              'Rules face - Vertical Screen - Vertical video - fill no'
            );
            // Si el video esta en vertical
            vid.style.maxWidth = '100%';
            vid.style.maxHeight = 'inherit';
          ...
          if (vu.sop.videoResizeStyleFillContainer) {
            ...
            proportionContainer =
              vidContainer.offsetHeight / vidContainer.offsetWidth;

            if (proportionVideo < proportionContainer) {
              console.log(
                'Rules face - Vertical Screen - Horizontal video - fill yes - 1'
              );
              vid.style.maxWidth = 'fit-content';
              vid.style.maxHeight = '100%';
              vid.style.width = 'auto';
              ...
              console.log(
                'Rules face - Vertical Screen - Horizontal video - fill yes - 2'
              );
              vid.style.maxWidth = '100%';
              vid.style.maxHeight = 'fit-content';
              vid.style.width = '100%';
            ...
            console.log(
              'Rules face - Vertical Screen - Horizontal video - fill no'
            );
            // Si el video esta en horizontal
            vid.style.maxWidth = 'inherit';
            vid.style.maxHeight = '100%';
        ...
        if (vu.camera.isVerticalVideo()) {
          console.log(
            'Rules face - Horizontal Screen - Vertical video - fill no'
          );
          // Si el video esta en vertical
          vid.style.maxWidth = '100%';
          vid.style.maxHeight = 'inherit';
          ...
          if (vu.sop.videoResizeStyleFillContainer) {
            console.log(
              'Rules face - Horizontal Screen - Horizontal video - fill yes'
            );
            vid.style.maxWidth = 'fit-content';
            vid.style.maxHeight = '100%';
            vid.style.width = 'auto';
            ...
            console.log(
              'Rules face - Horizontal Screen - Horizontal video - fill no'
            );
            vid.style.maxWidth = 'inherit';
            vid.style.maxHeight = '100%';
            vid.style.width = 'auto';
...
if (typeof vu.sop.steps == 'undefined') {
  vu.sop.steps = function () {};
}

if (typeof vu.sop.screenRecorder == 'undefined') {
  vu.sop.screenRecorder = function () {};
}
  ...
  if (vu.sop.recordProcess === true) {
    if (vu.sop.ui.isMobile()) {
      ...
      try {
        ...
        vu.sop.screenRecorder.recorder = new MediaRecorder(
          vu.sop.screenRecorder.stream
        );
        const chunks = [];
        vu.sop.screenRecorder.recorder.ondataavailable = (e) =>
          chunks.push(e.data);
        vu.sop.screenRecorder.recorder.onstop = (e) => {
          try {
            let videoType = chunks[0].type.split(';')[0];
            vu.sop.screenRecorder.completeBlob = new Blob(chunks, {
              type: 'video/mp4',
            });
      ...
      if (vu.sop.screenRecorder.recorder !== undefined) {
        if (vu.sop.screenRecorder.recorder.state != 'inactive') {
          ...
          if (vu.sop.screenRecorder.sendVideo === true) {
            ...
            return await vu.sop.steps.addVideoResolve(
              vu.sop.screenRecorder.completeBlob
            );
          } else {
            while (vu.sop.screenRecorder.videoReady === false) {
...
vu.sop.steps.document = async function () {
  let promise = new Promise(function (resolve, reject) {
    ...
    let divContainer = vu.sop.ui.documentSelectUploadMethodDraw(
      'vu.sop.steps.takePictureDocument()',
      'vu.sop.steps.uploadFrontDocumentPicture()'
    );
    document
      .getElementById('vu.sop.ui.documentSelectUploadMethod')
...
vu.sop.steps.uploadFrontDocumentPicture = async function () {
  ...
  document
    .getElementById('vu.sop.ui.documentFileUploadFront')
    .appendChild(divContainer);
...
vu.sop.steps.uploadFrontDocumentPictureResolve = async function (file) {
  while (true) {
    try {
      ...
      response = await vu.sop.api.addFront(
      ...
      if ('addDocumentPictureRequired' in response) {
        if (response.addDocumentPictureRequired) {
          ...
          if (!response.documentPictureDetected) {
            ...
            if (face !== null) {
              response = await vu.sop.api.addDocumentImage(
                vu.sop.userNameValue,
                vu.sop.operationIdValue,
                vu.sop.operationGuidValue,
                face
      ...
      await vu.error.showError(
        new vu.error.UploadDocumentFrontError(e.message)
      );
    }
  }
};
...
vu.sop.steps.uploadBackDocumentPicture = async function () {
  ...
  document
    .getElementById('vu.sop.ui.documentFileUploadBack')
    .appendChild(divContainer);
...
vu.sop.steps.uploadBackDocumentPictureResolve = async function (file) {
  while (true) {
    try {
      ...
      await vu.sop.ui.showWhiteLoading();
      response = await vu.sop.api.addBack(
        vu.sop.userNameValue,
        vu.sop.operationIdValue,
        vu.sop.operationGuidValue,
        backDocumentImg
...
vu.sop.steps.loadLibsAndCamera = async function () {
  if (vu.sop.warmUpDocModelAsync) {
    ...
    // Load network
    objectDetectionLoadPromise =
      await vu.sop.document.objectDetection.loadModel();
    // Load HAAR
  ...
  while (true) {
    try {
      ...
      if (vu.sop.flipDocumentCamera === 'auto') {
        if (vu.sop.ui.isMobile()) {
          console.log(
            'Flip Document configured to auto. Is mobile, mirroring screen'
          );
      ...
      vu.sop.videoResizeRules = 'doc';
      vu.sop.videoResizeObserver.observe(
        document.getElementById('vu.sop.ui.videoContainer')
      );
...
vu.sop.steps.addVideoResolve = async function (video) {
  await vu.sop.ui.showWhiteLoading();
  response = await vu.sop.api.addVideo(
    vu.sop.userNameValue,
    vu.sop.operationIdValue,
    vu.sop.operationGuidValue,
    video
...
vu.sop.steps.takePictureDocumentFront = async function () {
  ...
  vu.sop.document.ui.bgElement = document.getElementById(
    'vu.sop.document.ui.background'
  );
  vu.sop.document.ui.bgElement.style.backgroundImage =
    vu.sop.document.ui.bgInactive;
...
vu.sop.steps.authFace = async function () {
    ...
    try {
      ...
      if (vu.sop.enableSelfieList === true) {
        if (vu.face.ui.picturesTags)
          response = await vu.sop.api.registers(
            vu.sop.userNameValue,
            vu.sop.operationIdValue,
            vu.sop.operationGuidValue,
            pictures,
        ...
        response = await vu.sop.api.register(
          vu.sop.userNameValue,
          vu.sop.operationIdValue,
          vu.sop.operationGuidValue,
          picture
      ...
      response = await vu.sop.api.endOperation(
        vu.sop.userNameValue,
        vu.sop.operationIdValue,
        vu.sop.operationGuidValue
      );
  ...
  if (vu.sop.disableBiometric) {
    ...
    if (response.code != 1907 && response.code != 903) {
...
vu.sop.createLoadingImg = async function () {
  ...
  document
    .getElementById('vu.sop.ui.loadingImg')
    .appendChild(imgElem.cloneNode());
};
