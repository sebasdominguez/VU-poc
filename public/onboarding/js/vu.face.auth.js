...
if (typeof vu.face.auth == 'undefined') {
  vu.face.auth = function () {};
}
...
vu.face.auth.load = async function (basePath) {
  try {
    ...
    let webRTCadapter = vu.face.auth.loadJs(
    ...
    if (vu.face.useGestures == true) {
      ...
      let tfJsLoad = vu.face.auth.loadJs(
        basePath + '/js/libs/tensorflowjs/3.11.0/tf.min.js'
      );
      await tfJsLoad;
      let tfJsWasmLoad = vu.face.auth.loadJs(
      ...
      console.log(
        ...
        vu.face.auth.faceOrientationModelWeights
      );
    ...
    /* ----------------------------------------------------------------------------- */
    if (loadAudioLang) {
      audioLangLoad = vu.face.auth.loadJs(
        basePath + '/js/vu.sop.audio.' + vu.face.lang + '.js'
      );
    }
    let msgs = vu.face.auth.loadJs(
      basePath + '/js/vu.sop.msg.' + vu.face.lang + '.js'
    );
    ...
    let blurDetectionLoad = vu.face.auth.loadJs(
    ...
    let screenCapture = vu.face.auth.loadJs(
      basePath + '/js/vu.screen.capture.js'
    );
    let h264 = vu.face.auth.loadJs(
      basePath + '/js/libs/h264-mp4-encoder/h264-mp4-encoder.web.js'
    );
    let htm2canvas = vu.face.auth.loadJs(
    ...
    if (vu.face.useGestures == true) {
      ...
      faceUiGesturesLoad = vu.face.auth.loadJs(
        basePath + '/js/vu.face.ui.gestures.js'
      );
      faceLibLoad = vu.face.auth.loadJs(
        basePath + '/js/libs/face/jeelizFaceTransfer.js'
      );
    } else if (vu.face.useGestures == 'mixedChallenge') {
      console.log('Loading mixedChallenge mode');
      faceLoad = vu.face.auth.loadJs(
        basePath + '/js/vu.face.mixedChallenge.js'
      );
      faceObjectDetection = vu.face.auth.loadJs(
        basePath + '/js/vu.sop.face.objectDetectionAndRotation.js'
      );
      faceDirectionGesturesDetection = vu.face.auth.loadJs(
        basePath + '/js/vu.sop.face.model.directionsAndGestures.js'
      );
      faceMixedChallengeUi = vu.face.auth.loadJs(
...
vu.face.auth.start = async function () {
  while (true) {
    //try {
    await vu.sop.ui.showWhiteLoading();
    vu.face.auth.videoResizeObserver.observe(
      ...
      if (vu.face.auth.registrationFlow) {
        if (vu.face.auth.enableSelfieList === true) {
          if (vu.face.ui.picturesTags.length > 0)
            response = await vu.sop.api.faceRegisters(
              vu.face.auth.userNameValue,
              pictures,
          ...
          response = await vu.sop.api.faceRegister(
            vu.face.auth.userNameValue,
            lastPic
          );
        }
        ...
        if (vu.face.auth.enableSelfieList === true) {
          if (vu.face.ui.picturesTags.length > 0)
            response = await vu.sop.api.faceLoginList(
              vu.face.auth.userNameValue,
              pictures,
              vu.face.ui.picturesTags
            );
          ...
          response = await vu.sop.api.faceLogin(
            vu.face.auth.userNameValue,
            lastPic
          );
...
vu.face.auth.loadingImgSrcDefault =
